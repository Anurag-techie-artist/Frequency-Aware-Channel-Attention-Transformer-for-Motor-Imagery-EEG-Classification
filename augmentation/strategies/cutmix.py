"""
CutMix EEG Data Augmentation Strategy Implementation.
"""
...
class CutMixStrategy(AugmentationStrategy):
    """CutMix patch swapping strategy for EEG spectral tensors."""
    ...
    def generate(self, num_samples: int, num_classes: int = 4) -> SyntheticDataset:
        ...
        return SimpleSyntheticDataset(empty_x, empty_y, {"strategy": "cutmix"})
    ...
    def augment(
        ...
        cut_x = real_x[idx1].clone()
        # Cut second half of time samples from idx2
        cut_x[:, :, :, 125:] = real_x[idx2][:, :, :, 125:]
        cut_y = real_y[idx1]

        aug_x = torch.cat([real_x, cut_x], dim=0)
        aug_y = torch.cat([real_y, cut_y], dim=0)
        return aug_x, aug_y
